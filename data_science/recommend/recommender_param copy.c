...
void predict_test(const char *test_file, const char *output_file)
{
    ...
    if (!fin || !fout)
    {
        perror("Test or output file error");
        exit(1);
    }
...
int main(int argc, char *argv[])
{
    if (argc != 7)
    {
        printf("Usage: %s training_file test_file K learning_rate lambda epochs\n", argv[0]);
        return 1;
    }

    // P: [num_users][K]
    P = malloc(num_users * sizeof(double *));
    for (int i = 0; i < num_users; i++)
        P[i] = malloc(K * sizeof(double));

    // Q: [num_items][K]
    Q = malloc(num_items * sizeof(double *));
    for (int i = 0; i < num_items; i++)
        Q[i] = malloc(K * sizeof(double));

    const char *training_file = argv[1];
    const char *test_file = argv[2];
    K = atoi(argv[3]);
    LEARNING_RATE = atof(argv[4]);
    REGULARIZATION = atof(argv[5]);
    EPOCHS = atoi(argv[6]);

    char output_file[100];
    snprintf(output_file, sizeof(output_file), "%.*s.base_prediction.txt",
             (int)(strchr(training_file, '.') - training_file), training_file);
